...
SHORT_VERSION= "$(echo $VERSION_NUMBER | grep -Po '.*(?=\.)')"
...
DOCKERHUB_TAGS=(--tag "${DOCKERHUB_REPOSITORY}:${RELEASE_VERSION}" --tag "${DOCKERHUB_REPOSITORY}:${SHORT_VERSION}")
DOCKERHUB_UBI_TAGS=(--tag "${DOCKERHUB_REPOSITORY}:${RELEASE_VERSION}-ubi7" --tag "${DOCKERHUB_REPOSITORY}:${SHORT_VERSION}-ubi7")
